...
function fileExist(){
$exist =     
    (Test-Path ("C:\ProgramData\InfoShare.Deployment\v" + $deploy.SoftwareVersion +"\"+ $deploy.Name  + "\Backup\Web\Author\ASP\Editors\Xopus\config\config.xml")) -and
    (Test-Path ("C:\ProgramData\InfoShare.Deployment\v" + $deploy.SoftwareVersion +"\"+ $deploy.Name  + "\Backup\Web\Author\ASP\Editors\Xopus\config\bluelion-config.xml")) -and
    (Test-Path ("C:\ProgramData\InfoShare.Deployment\v" + $deploy.SoftwareVersion +"\"+ $deploy.Name  + "\Backup\Web\Author\ASP\XSL\FolderButtonbar.xml")) -and
    (Test-Path ("C:\ProgramData\InfoShare.Deployment\v" + $deploy.SoftwareVersion +"\"+ $deploy.Name  + "\Backup\Web\Author\ASP\XSL\InboxButtonBar.xml")) -and
    (Test-Path ("C:\ProgramData\InfoShare.Deployment\v" + $deploy.SoftwareVersion +"\"+ $deploy.Name  + "\Backup\Web\Author\ASP\XSL\LanguageDocumentButtonbar.xml")) -and
    (Test-Path ("C:\ProgramData\InfoShare.Deployment\v" + $deploy.SoftwareVersion +"\"+ $deploy.Name  + "\Backup\Web\Author\ASP\Web.config"))

 return $exist   
}

 Remove-Item ("C:\ProgramData\InfoShare.Deployment\ISH"+$deploy.OriginalParameters.projectsuffix ) -Recurse -Force -ErrorAction SilentlyContinue
...
function VanilaBackupWhenCommandletNotSuccesful_test{
 Remove-Item ("C:\ProgramData\InfoShare.Deployment\v" + $deploy.SoftwareVersion +"\"+ $deploy.Name ) -Recurse -Force -ErrorAction SilentlyContinue
 ...
 try{
  Enable-ISHUIQualityAssistant -ISHDeployment $deploy  -ErrorAction Stop
    ...
    $checkResult = Test-Path ("C:\ProgramData\InfoShare.Deployment\v" + $deploy.SoftwareVersion +"\"+ $deploy.Name  + "\Backup\Web\Author\ASP\Editors\Xopus\config\config.xml")
