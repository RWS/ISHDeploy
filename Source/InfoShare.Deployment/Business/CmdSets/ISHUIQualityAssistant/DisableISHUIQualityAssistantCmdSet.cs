...
namespace InfoShare.Deployment.Business.CmdSets.ISHUIQualityAssistant
{
    public class DisableISHUIQualityAssistantCmdSet : ICmdSet
    {
        ...
        public DisableISHUIQualityAssistantCmdSet(ILogger logger, ISHProject ishProject)
        {
			...
			_invoker.AddCommand(new XmlNodeCommentCommand(logger, Path.Combine(ishProject.AuthorFolderPath, ISHPaths.EnrichConfig), _uncommentPatterns));
			_invoker.AddCommand(new XmlNodeCommentCommand(logger, Path.Combine(ishProject.AuthorFolderPath, ISHPaths.XopusConfig), _uncommentPatterns));
		}
