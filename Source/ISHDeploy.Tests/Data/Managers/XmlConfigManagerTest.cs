    ...
    public class XmlConfigManagerTest : BaseUnitTest
    {
		...
		public void SetNode_New()
		{
			...
			FileManager.Save(_filePath, Arg.Do<XDocument>(
				xdoc =>
				{
					comment = ((IEnumerable<object>)xdoc.XPathEvaluate($"{testXPath}{BasePathsOperation.EventMonitorMenuBarXml.EventMonitorPreccedingCommentXPath}")).OfType<XComment>().Single();
		...
		public void MoveBeforeNode()
		{
			...
			_xmlConfigManager.MoveBeforeNode(
				_filePath, 
				string.Format(BasePathsOperation.EventMonitorMenuBarXml.EventMonitorTab, testLabel), 
				string.Format(BasePathsOperation.EventMonitorMenuBarXml.EventMonitorTab, insertBeforeLabel));
