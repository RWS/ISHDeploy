...
namespace InfoShare.Deployment.Business.CmdSets.ISHExternalPreview
{
    public class DisableISHExternalPreviewCmdSet : ICmdSet
    {
        ...
        public DisableISHExternalPreviewCmdSet(ILogger logger, Models.ISHDeployment ishDeployment)
        {
            _invoker = new CommandInvoker(logger, "InfoShare ExternalPreview deactivation");

            _invoker.AddCommand(new XmlSetAttributeValueCommand(logger, Path.Combine(ishDeployment.AuthorFolderPath, ISHPaths.WebConfig), CommentPatterns.TrisoftInfoshareWebExternalXPath, CommentPatterns.TrisoftInfoshareWebExternalAttributeName, "THE_FISHEXTERNALID_TO_USE"));
            _invoker.AddCommand(new XmlNodeCommentCommand(logger, Path.Combine(ishDeployment.AuthorFolderPath, ISHPaths.WebConfig), _commentPatterns));
        }
